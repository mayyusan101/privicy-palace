...
const handleSubmitFrom = async() => {
    ...
    if(!validate) return; // fail validation
    try{
      ...
      notify({type:"success", message: "Login success", theme:"dark"});
    }catch(error){
      // Handle errors
        ...
        <div class="flex flex-col items-start min-w-[100%] sm:min-w-[80%] md:min-w-[60%] px-7 py-7  bg-bgChat rounded-lg">
          ...
          <div class=" w-full text-end">
            <span class="cursor-pointer">
              <router-link :to="{ name:'register' }" class="text-xs underline text-slate-50">
